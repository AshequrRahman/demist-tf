...
def train(base_folder, weights_name, loss_fn_name, dose_level, num_iter, bt_size, num_epochs, learning_folder, 
  ...
  for ind_fold, (train_pat_index, test_pat_index) in enumerate(kf.split(np.arange(num_pat))):
    ...
    train_dataset = tf.data.Dataset.from_tensor_slices((X_train, Y_train, Y_train_loc))
    train_dataset = train_dataset.shuffle(buffer_size=len(X_train)).batch(bt_size)
    val_dataset = tf.data.Dataset.from_tensor_slices((X_test, Y_test, Y_test_loc)).batch(bt_size)
    
    #===================================
    # custom loop
    #===================================
    denoise_obs_net = build_dennet3D(input_shape, loc_shape, num_reg, lambda_val_chdiff, U)
    com_flag = f'_d{dose_level}_it{num_iter}_b{bt_size}_lmbdchdiff{lambda_val_ind_chdiff}_f{ind_fold}'
    ...
    for epoch in range(num_epochs):
      ...
      if val_flag:
        ...
        denoise_obs_net.reset_metrics()
        loss_value = 0
        for step, (x_batch_val, y_batch_val, y_batch_loc_val) in enumerate(val_dataset):
          loss_value += val_step(denoise_obs_net, x_batch_val, y_batch_val, y_batch_loc_val)
    ...
    #======================================================================================================================================
    # Plot loss and metric curve
    #======================================================================================================================================
    # loss
    png_filename = f'{result_folder}/losses/{loss_curve_name}.png'
    ...
    # mse
    png_filename = f'{result_folder}/losses/{loss_curve_name}_mse.png'
    log_plot_loss_metric(
                          train_mse, val_mse, 
    ...
    # chv_mse
    png_filename = f'{result_folder}/losses/{loss_curve_name}_chv_mse.png'
    log_plot_loss_metric(
  ...
  #======================================================================================================================================
  # 4. save best validation loss epoch
  #======================================================================================================================================
  ...
  fname = f'{result_folder}/losses/best_epoch_{com_flag}'
